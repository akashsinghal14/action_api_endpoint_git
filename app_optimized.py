...
print(f"Optimized app initialized with Claude model: {DEFAULT_CLAUDE_MODEL}")
print(f"Caching enabled: {ENABLE_CACHING}, TTL: {CACHE_TTL/3600} hours")
...
def create_optimized_claude_prompt(survey_data):
    """Create optimized prompt for Claude - 60% smaller"""
    data_lines = []
...
async def analyze_gap_with_claude_async(gap_type, value, unit, api_key, model):
    """Optimized async Claude analysis with caching"""
    try:
        # Create cache key
        cache_key = f"{gap_type}:{value}:{unit}:claude:{model}"
        
        # Check cache first
        if ENABLE_CACHING:
            cached_response = cache.get(cache_key)
            if cached_response:
                print(f"Cache HIT for {gap_type}: {value}")
                return cached_response
        
        print(f"Cache MISS for {gap_type}: {value} - calling Claude API")
        ...
        prompt = create_optimized_claude_prompt(survey_data)
        
        # Rate limiting check
        if not rate_limiter.can_make_call():
            raise Exception('Rate limit exceeded')
        
        # Claude API call with aiohttp for better async performance
        ...
        if ENABLE_CACHING:
            ...
            print(f"Cached response for {gap_type}: {value}")
        
        return response_data
        
    except Exception as e:
        print(f"Error in analyze_gap_with_claude_async: {e}")
        import traceback
        print(f"Traceback: {traceback.format_exc()}")
        return None

# API Endpoints - Same structure as original

@app.route('/api/action_item/head', methods=['POST'])
@app.route('/api/action_item/head/<value>/<unit>', methods=['GET'])
async def slim_head(value=None, unit=None):
    try:
        ...
        if api_key and not is_compliant:
            ...
            if real_key:
                ai = await analyze_gap_with_claude_async('head', value, unit, real_key, model)
                if ai:
                    return jsonify(ai)
        ...
        return jsonify({
            ...
            'ai_provider': 'claude' if (api_key and not is_compliant) else None,
        })
    except Exception as e:
        return jsonify({'error': f'Head gap analysis failed: {str(e)}'}), 500

# Add other endpoints here (hinge, closing, threshold, etc.) - same pattern as head
...
async def analyze_measurement():
    """Unified endpoint for all fire door measurements with Claude support"""
    try:
        data = await request.get_json() or {}
        ...
        api_key = data.get('api_key')
        model = data.get('model')
        ai_provider = data.get('ai_provider', 'claude')  # Default to Claude
        ...
        if measurement_type in numeric_types:
            ...
            if measurement_type == 'head':
                return await handle_numeric_measurement_unified('head', value, unit, api_key, model, 4, 'max_allowed')
            elif measurement_type == 'hinge':
                return await handle_numeric_measurement_unified('hinge', value, unit, api_key, model, 4, 'max_allowed')
            elif measurement_type == 'closing':
                return await handle_numeric_measurement_unified('closing', value, unit, api_key, model, 4, 'max_allowed')
            elif measurement_type == 'threshold':
                return await handle_numeric_measurement_unified('threshold', value, unit, api_key, model, 4, 'max_allowed')
            elif measurement_type == 'doorthick':
                return await handle_numeric_measurement_unified('door_thickness', value, unit, api_key, model, 44, 'min_required')
            elif measurement_type == 'framedepth':
                return await handle_numeric_measurement_unified('frame_depth', value, unit, api_key, model, 100, 'min_required')
            elif measurement_type == 'doorsize':
                return await handle_numeric_measurement_unified('door_size', value, unit, api_key, model, 500, 'min_required')
        ...
        # Handle boolean measurements
            ...
            if measurement_type == 'intustrips':
                return await handle_boolean_measurement_unified('intumescent_strips', value, api_key, model, 'high')
            elif measurement_type == 'selfclosing':
                return await handle_boolean_measurement_unified('self_closing_device', value, api_key, model, 'high')
            elif measurement_type == 'shutsign':
                return await handle_boolean_measurement_unified('keep_shut_sign', value, api_key, model, 'medium')
            elif measurement_type == 'holddevice':
                return await handle_boolean_measurement_unified('hold_open_device', value, api_key, model, 'medium')
            elif measurement_type == 'certivisible':
                return await handle_boolean_measurement_unified('certification_visible', value, api_key, model, 'high')
            elif measurement_type == 'glazing':
                return await handle_boolean_measurement_unified('contains_glazing', value, api_key, model, 'medium')
            elif measurement_type == 'pyroglazing':
                return await handle_boolean_measurement_unified('pyro_glazing', value, api_key, model, 'high')
            elif measurement_type == 'doorclosefully':
                return await handle_boolean_measurement_unified('door_close_fully', value, api_key, model, 'high')
            elif measurement_type == 'hingesfirerated':
                return await handle_boolean_measurement_unified('hinges_fire_rated', value, api_key, model, 'high')
        
    except Exception as e:
        return jsonify({'error': f'Analysis failed: {str(e)}'}), 500

async def handle_numeric_measurement_unified(gap_type, value, unit, api_key, model, threshold, threshold_type):
    ...
    try:
        ...
        if api_key and not is_compliant:
            ...
            real_key = resolve_api_key(api_key)
            if real_key:
                ai = await analyze_gap_with_claude_async(gap_type, value, unit, real_key, model)
                if ai:
                    return jsonify(ai)
...
async def handle_boolean_measurement_unified(measurement_type, value, api_key, model, default_severity):
    ...
    try:
        ...
        if api_key and not is_compliant:
            ...
            real_key = resolve_api_key(api_key)
            if real_key:
                ai = await analyze_gap_with_claude_async(measurement_type, value, 'boolean', real_key, model)
                if ai:
                    return jsonify(ai)
                action_items = []
        else:
...
@app.route('/')
async def serve_comparison_ui():
    """Serve the comparison UI HTML file"""
    return await send_file('comparison_ui.html')

@app.route('/comparison')
async def serve_comparison_ui_alt():
